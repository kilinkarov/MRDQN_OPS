...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim: int, size: int, batch_size: int, n_step: int, gamma: float):
    ...
    def sample_batch_from_idxs(self, idxs: np.ndarray) -> Dict[str, np.ndarray]:
        """Sample batch from specific indices."""
        return dict(
            obs=self.obs_buf[idxs],
            next_obs=self.next_obs_buf[idxs],
            acts=self.acts_buf[idxs],
    ...
    def _get_n_step_info(
        ...
        for transition in reversed(list(n_step_buffer)[:-1]):
            step_reward, step_next_obs, step_done = transition[-3:]

            rew = step_reward + gamma * rew * (1 - step_done)
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def _sample_proportional(self) -> List[int]:
        ...
        for i in range(self.batch_size):
            ...
            upperbound = random.uniform(segment_start, segment_end)
