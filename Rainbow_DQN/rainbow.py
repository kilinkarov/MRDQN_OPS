...
class Network(nn.Module):
    def __init__(
        ...
        self.feature_layer = nn.Sequential(
            nn.Linear(in_dim, 512),  # ✅ Увеличиваем до 512 как в оригинале
            nn.ReLU(),
        )

        # set advantage layer
        self.advantage_hidden_layer = NoisyLinear(512, 512)  # ✅ 512 hidden units
        self.advantage_layer = NoisyLinear(512, out_dim * atom_size)

        # set value layer
        self.value_hidden_layer = NoisyLinear(512, 512)  # ✅ 512 hidden units
...
class DQNAgent:
    ...
    def __init__(
        ...
        # optimizer
        self.optimizer = optim.Adam(self.dqn.parameters(), lr=6.25e-5, eps=1.5e-4)  # ✅ Оригинальные параметры
