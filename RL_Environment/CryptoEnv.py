...
class CryptoEnv(gym.Env):
    ...
    def getPriceList(self):
        ...
        refer_value = original_observations[0][-1]  # ✅ Первая цена эпизода как референс
    ...
    def reset(self, seed=None, options=None):
        ...
        # ✅ ИСПРАВЛЯЕМ position_value БЕЗ заглядывания в будущее
        price_diff = self.original_price_list[self.t] - self.refer_price
        # Используем только цены ДО ТЕКУЩЕГО дня включительно
        prices_so_far = self.original_price_list[:self.t+1]
        typical_range = max(prices_so_far) - min(prices_so_far) if len(prices_so_far) > 1 else abs(self.refer_price * 0.1)
        if typical_range > 0:
            normalized_diff = price_diff / typical_range
        else:
            normalized_diff = 0
        self.position_value = sigmoid(normalized_diff)  # ref value
